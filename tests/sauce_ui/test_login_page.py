...
def test_login_with_valid_credentials(sauce_ui, username, password):
    ...
    assert sauce_ui.page.url.endswith("/inventory.html"), f"Expected redirect to inventory after login with {username}"
...
def test_login_with_performance_glitch_user(sauce_ui):
    ...
    assert sauce_ui.page.url.endswith("/inventory.html"), "Expected redirect to inventory after login with performance glitch user"
...
def test_login_with_invalid_credentials(sauce_ui, username, password, expected_error):
    ...
    sauce_ui.login_page.login(username, password)
    assert expected_error in sauce_ui.login_page.error_message, f"Expected error message containing '{expected_error}' for login with {username}:{password}, got: '{sauce_ui.login_page.error_message}'"
    assert sauce_ui.page.url == initial_url, f"Expected no redirect after failed login with {username}"